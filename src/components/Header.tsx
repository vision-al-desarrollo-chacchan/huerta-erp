...
import { getCurrentStaffName } from "../services/restaurant-store";
...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("huerta-settings-updated", load);
  }, []);
      ...
      <div className="flex items-center gap-5 ml-auto">
        ...
        <div className="flex items-center gap-3 border-r border-slate-200 dark:border-slate-700 pr-5">
          ...
          <button className="relative p-2 text-slate-500 hover:text-brand-blue hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-colors">
            ...
            <span className="absolute top-1 right-1 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white dark:border-brand-surfaceDark">
              4
            </span>
          </button>
          <button
