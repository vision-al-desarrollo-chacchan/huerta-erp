...
export default function Arqueo() {
  ...
  return <div className="space-y-5 p-4 text-slate-900 dark:text-white lg:p-6">
    ...
    <div><h1 className="text-3xl font-black">Arqueo de inventario</h1><p className="text-sm text-slate-500">Cuenta los insumos al final del turno y compara con el stock que registró el sistema. El arqueo de efectivo sigue en Caja.</p></div>
    ...
    {loading ? <p>Cargando arqueos…</p> : <>
      ...
      <label className="block">Observaciones<textarea disabled={audit.estado === 'cerrado'} value={notes} onChange={e => setNotes(e.target.value)} className="mt-2 block min-h-20 w-full rounded-xl border p-3 text-slate-900" placeholder="Motivo de las diferencias, mermas o incidencias" /></label>
      <div className="flex flex-wrap gap-3">{audit.estado === 'abierto' && <button disabled={busy || !complete || !cashId || audit.caja_id !== cashId} onClick={() => void close()} className="rounded-xl bg-slate-900 px-5 py-3 font-bold text-white disabled:opacity-50">Cerrar arqueo</button>}<button disabled={!complete} onClick={() => void share().catch(e => setError(String(e.message ?? e)))} className="rounded-xl bg-emerald-600 px-5 py-3 font-bold text-white disabled:opacity-50">Compartir reporte</button><button disabled={!complete} onClick={() => window.print()} className="rounded-xl border px-5 py-3 font-bold">Imprimir / PDF</button></div></>}
    </>}
  </div>;
}
