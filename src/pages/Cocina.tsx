            ...
            <div className="space-y-3">
              {activeOrders.filter((order) => order.status === column.status).map((order) => (
                <article key={order.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                  ...
                  {order.activeAdditionId && <p className="mt-3 rounded-lg bg-amber-100 px-3 py-2 text-xs font-black uppercase text-amber-800">Adición de {order.table}</p>}
                  <ul className="my-4 space-y-2 border-y border-slate-100 py-3 dark:border-slate-700">{order.items.filter((item) => !order.activeAdditionId || item.additionId === order.activeAdditionId).map((item,index) => <li key={`${item.productId}-${index}`} className="flex gap-2 text-sm text-slate-700 dark:text-slate-200"><b className="text-orange-500">{item.quantity}×</b>{item.name}</li>)}</ul>
