      ...
      {closedReport && <>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4">
          <div id="cash-close-receipt" className="max-h-[92vh] w-full max-w-sm overflow-y-auto rounded-2xl bg-white p-6 text-slate-950 shadow-2xl">
            ...
            <div className="my-3 border-y-2 border-black py-2"><ReceiptRow label="VENTA TOTAL DEL DÍA" value={money.format(['Efectivo','Yape/Plin','Tarjeta','Transferencia'].reduce((total, method) => total + (closedReport.payments[method] ?? 0), 0))} strong /></div>
            ...
            <div className="mt-3 border-t border-dashed border-slate-400 pt-2"><ReceiptRow label="MOVIMIENTO NETO DEL DÍA" value={money.format(['Efectivo','Yape/Plin','Tarjeta','Transferencia'].reduce((total, method) => total + (closedReport.payments[method] ?? 0), 0) + closedReport.movements.reduce((total, item) => total + (item.type === 'ingreso' ? item.amount : -item.amount), 0))} strong /></div>
